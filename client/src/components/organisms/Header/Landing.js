...
const Mobile = ({handleSearch, displayButtons, search}) => {
  return (
    <header className='header--mobile'>
      <div className='header__inner--mobile'>
        ...
        <div className='header__middle--mobile'>
          <Input type='landing' data={search} setData={handleSearch} />
          <div className='header__input'>
            <Input type='landing' data={search} setData={ handleSearch } placeholder='何をお探しですか？'  />
            <button>
              <svg
                aria-hidden='true'
                width='16'
...
const Tablet = ({ handleSearch, displayButtons, search }) => {
...
export const LandingHeader =()=>{
  ...
  // オブジェクトが入れうとわかっていればからのオブジェクトを初期値に設定
  ...
  useEffect(() => {
    ...
    //以下は非同期型の書き方(コールバックでエラー処理を書く)
    jwt.verify(token, process.env.REACT_APP_JWT_KEY, (err, decoded) => {
      // undefined null
      // もし途中でエラーが起こってしまったりtokenが一致しなければstateをfalseに
      if (err || !decoded) {
        setIsLoggedIn(false)
        return
      }
